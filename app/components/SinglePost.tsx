...
export default function SinglePost({ titulo, autor, fecha, thumbnail, pid }) {
  ...
  <div className="flex flex-col">
    ...
    <div className="text-black bg-green-300/50"><Link href={`${process.env.NEXT_PUBLIC_APP_URL}/perfil/${autor}`}>Autor: <span className=" font-semibold">{autor}</span></Link></div>
  </div>
  );
}
